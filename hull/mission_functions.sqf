...
hull_mission_fnc_readMissionParamValues = {
    if (!isNil {hull_mission_date}) then {
        hull_mission_date = (getArray (missionConfigFile >> "Hull_MissionParams" >> "date")) select hull_mission_date;
    };
    if (!isNil {hull_mission_timeOfDay}) then {
        hull_mission_timeOfDay = (getArray (missionConfigFile >> "Hull_MissionParams" >> "time")) select hull_mission_timeOfDay;
    };
    if (!isNil {hull_mission_weather}) then {
        hull_mission_weather = (getArray (missionConfigFile >> "Hull_MissionParams" >> "weather")) select hull_mission_weather;
    };
};
